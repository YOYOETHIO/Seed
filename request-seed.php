...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        // Get file names (optional, for reference)
        $compfile = $_FILES["compfile"]["name"];
        $cocfile = $_FILES["cocfile"]["name"];
		

        // Read file contents as binary (BLOB)
        $compfile = mysqli_real_escape_string($bd, file_get_contents($_FILES["compfile"]["tmp_name"]));
        $cocfile = mysqli_real_escape_string($bd, file_get_contents($_FILES["cocfile"]["tmp_name"]));

        // Insert main complaint record with binary files
        $query=mysqli_query($bd, "insert into tblcomplaints(userId,	noc,complaintDetails,complaintFile,request_date,seed_producer,	telephone,Person_Name,Region,email,cocdoc,mode,woreda,zones) values('$uid','$noc','$complaintdetials','$compfile','$R_date','$producer','$telephone','$person_name','$Region','$email','$cocfile','$agree','$woreda','$zone')");
        ...
        for ($i = 0; $i < count($_POST['slno']); $i++) {
            ...
            if ($category && $subcategory && $complaintype && $amount) {
                $sql = "INSERT INTO amount_detail (request, crop, variety, class, amount)
                        VALUES ('$l_id', '$category', '$subcategory', '$complaintype', '$amount')";
                $stmt = $con->prepare($sql);
                $stmt->execute();
                $last_id = $con->insert_id;

                // Inventory logic (optional, adjust as needed)
                mysqli_query($bd, "
                    INSERT INTO inventory (product_id, qty, type, stock_from, form_id)
